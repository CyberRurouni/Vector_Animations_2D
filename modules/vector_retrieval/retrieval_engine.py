...
class VectorRetrievalEngine:
    ...
    def __init__(self, api_key: str):
        ...
        if not os.path.exists(self.download_path):
            ...
            logger.info(f"📂 [SYSTEM] Initialized vector vault at: {self.download_path}")

    def get_black_assets(self, search_term: str, count: int = 1) -> list:
        """
        ...
        Returns:
            list: Paths to the downloaded black PNG files.
        """
        local_files = []
        ...
        try:
            ...
            if not results:
                logger.warning(f"⚠️  [EMPTY] No matching vectors found for '{search_term}'")
                return []

            for item in results:
                icon_id = item['id']
                logger.info(f"🛰️  [LOCKED] Vector ID {icon_id} selected. Fetching download link...")
